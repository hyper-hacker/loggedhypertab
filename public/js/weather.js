class Info {
    ...
    static async getIpLoc() {
        let data = await this.getResponse(`http://ip-api.com/json/${ip}`);
...
async function doStuff() {
    ...
    if (!weather.alert.length == 0) {
        document.getElementById('alerts').setAttribute('onclick', `location.href = 'https://forecast.weather.gov/MapClick.php?&lat=${ipdat.latlon.replace(',', '&lon=')}'`);
    } else {
        return;
    };
}
